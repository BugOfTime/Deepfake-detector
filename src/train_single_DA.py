...
num_classes   = 2           #
num_workers = 16  #train 28, train 0
...
def main():
    ...
    hot = hot_config(path="single_hot_config.json", defaults=hot_defaults)

    tool = train_tool(
        trainer=trainer,
        epochs=epochs,  # Initial value (can be overridden by hot's max_epochs)
        model_type="single",
        log_dir="DA_log/single/continue",
        save_dir="DA_saved_model/single/continue",
        monitor="auc",
        patience=5,
        min_delta=1e-4,
        lr_factor=0.5,
