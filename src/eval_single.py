...
def to_1d_mask(mask_i, L: int) -> np.ndarray:
    """
    把任意形状的 pad/mask 压成长度为 L 的一维布尔向量。
    约定：True 表示“是 padding，需要屏蔽”。
    """
    m = np.asarray(mask_i)
    m = np.squeeze(m)  # 去掉所有1维
    if m.ndim == 0:
    ...
    if m.ndim == 1:
        ...
        if m.shape[0] != L:
            ...
            mm[:min(L, m.shape[0])] = m[:min(L, m.shape[0])]
            return mm.astype(bool)
        return m.astype(bool)

    # ndim >= 2: 找到长度为 L 的轴
    axes = [ax for ax, s in enumerate(m.shape) if s == L]
    if axes:
        # 把该轴移到最后，并在其它轴取第 0 个切片
        m = np.moveaxis(m, axes[0], -1)
        while m.ndim > 1:
            m = m[0]
        return m.astype(bool)
    ...
    m = m.reshape(-1)
    mm = np.zeros(L, dtype=bool)
    mm[:min(L, m.size)] = m[:min(L, m.size)]
    return mm.astype(bool)
...
def build_top_idx_list(attn_T, pad_mask=None):
    ...
    top_idx_list = []
    for i in range(N):
        att = attn_T[i].copy()  # 形状 [L]
        if pad_mask is not None:
            mask1d = to_1d_mask(pad_mask[i], L)  # 规范成 [L] 的 bool
            # 若 mask 全 True，说明可能语义相反（True=有效位置），尝试自动翻转一次
            if mask1d.all():
                mask1d = ~mask1d
            att[mask1d] = -np.inf  # 把 padding 位置屏蔽掉

        order = np.argsort(-att)              # 从大到小的索引
        valid = ~np.isneginf(att[order])      # 过滤被屏蔽的位置
        order = order[valid]
        top_idx_list.append(order.astype(int))
    return top_idx_list
